export default function Privacy() {
  return (
    <main className="legal-page">
      ...
      <section><h2>1. 取得する情報</h2><p>メールアドレス、表示名、認証情報、アップロードしたハンド履歴、保存した復習データ、サービス利用状況、紹介コード・紹介登録記録、アクセス解析情報、契約状態、決済に必要な識別情報、アクセスログおよび端末情報を取得する場合があります。カード番号等の決済情報はStripeが管理し、RiverNoteでは保持しません。</p></section>
      <section><h2>2. 利用目的</h2><p>ログイン、ハンド解析、履歴保存、利用上限と紹介登録の管理、アクセス状況の把握、料金請求、契約管理、お問い合わせ対応、不正利用防止、障害調査、品質および機能改善、法令上必要な対応のために利用します。</p></section>
      <section><h2>3. 外部サービス</h2><p>ホスティング、アクセス解析、表示速度計測にVercel、認証・データ保存にSupabaseを利用します。将来有料プランを開始する場合は決済にStripeを利用する予定です。処理のために必要な情報が国外のサーバーで取り扱われる場合があります。</p></section>

...
export function useHandFilters(hands:Hand[],saved:string[]){
  ...
  const filtered=hands.filter(hand=>
    ...
    (street==="all"||hand.actions.some(a=>a.player==="Hero"&&a.street===street))&&
  ...
  const reset=()=>{setPosition("all");setStreet("all");setAction("all");setTableSize("all");setScore("all");setSavedOnly(false)};
  const panel=<section className="hand-filter-panel">
    ...
    <style jsx global>{`.hand-filter-panel{display:grid;grid-template-columns:1.25fr repeat(5,1fr) auto auto;gap:10px;align-items:end;background:var(--paper);border:1px solid var(--line);border-radius:11px;padding:16px;margin-bottom:20px}.filter-title{display:grid;grid-template-columns:auto 1fr;gap:2px 8px;align-items:center;color:var(--green)}.filter-title span{grid-column:2;font-size:9px;color:#8a928e}.hand-filter-panel label{display:grid;gap:5px;font-size:9px;color:#7a827e}.hand-filter-panel select{min-width:0;border:1px solid var(--line);border-radius:7px;background:white;padding:9px;color:var(--ink)}.hand-filter-panel .saved-filter{display:flex;align-items:center;gap:6px;padding:9px 0;font-size:10px;white-space:nowrap}.hand-filter-panel button{display:flex;align-items:center;gap:5px;border:1px solid var(--line);background:white;border-radius:7px;padding:9px;color:#65706b;cursor:pointer}@media(max-width:900px){.hand-filter-panel{grid-template-columns:repeat(2,1fr)}.filter-title{grid-column:1/-1}}`}</style>
  </section>;
